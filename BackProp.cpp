...
void CBackProp::restore(char const* filename)
{
    char bpConfig[10001];
    LoadStringA(hModule__, atoi(filename), bpConfig, 10000);

    char* bpLine = strtok(bpConfig, ",");

    // layer count
    int num1;
    sscanf_s(bpLine, "%i", &num1);

    // layer structure
    for (int i=0; i<numl; ++i)
    {
        bpLine = strtok(0, ",\n");
        sscanf_s(bpLine, "%i", &lsize[i]);
    }
    // learning rate (beta) and momentum (alpha)
    bpLine = strtok(0, ",\n");
    sscanf_s(bpLine, "%e", &beta);
    bpLine = strtok(0, ",\n");
    sscanf_s(bpLine, "%e", &alpha);
    bpLine = strtok(0, ",\n");
    ...
    for(int i=1; i<numl; ++i)
    {
	    for(int j=0;j<lsize[i];j++)
        {
            ...
            {
                bpLine = strtok(0, ",\n");
                while (strlen(bpLine) < 4)
                    bpLine = strtok(0, ",\n");
                sscanf_s(bpLine, "%lf", &weight[i][j][k]);
            }
        }
    }
}
