...
size_t Buffer::Write(vector<uint8_t> const& writeBuffer)
{
    EnterCriticalSection(&mutex_);

    if (writeBuffer.size() == 65536)
        int j = 9;
    ...
    if (transferSize > freeSize)
    {
        ...
        {
            deque<uint8_t>::iterator beginIter = buffer_.begin();
...
size_t Buffer::Write(char const* writeBuffer, size_t writeCount)
{
    vector<uint8_t> writeVector(writeBuffer, writeBuffer+writeCount);
    return Write(writeVector);
}


size_t Buffer::Read(vector<uint8_t>& readBuffer, size_t readCount)
{
...
bool Buffer::IsFull()
{
    return (buffer_.size() >= bufferMax_);
}


void Buffer::Pop(uint8_t& popValue)
{
    EnterCriticalSection(&mutex_);
    popValue = buffer_.back();
    buffer_.pop_back();
    LeaveCriticalSection(&mutex_);
}